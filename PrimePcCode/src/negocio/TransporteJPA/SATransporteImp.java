...
public class SATransporteImp implements SATransporte {
	...
	public int bajaTransporte(int id) {
		...
		try {
			...
			if (t == null && t.getActivo() == 1) {
				 ...
				 * 
				 * 
				 * if(vinculaciones.isEmpty()) { t.setActivo(1); em.getTransaction().commit();
				 * res = t.getId(); }else { em.getTransaction().rollback(); }
				 */
		...
		try {
			...
			if (tExistente != null && tExistente.getActivo() == 1) {
				...
				List<Transporte> tMatricula = em
						.createNamedQuery("Negocio.TransporteJPA.Transporte.finfByMatricula", Transporte.class)
						.setParameter("matricula", t.getMatricula()).getResultList();
	...
	public Set<TTransporte> leerTodosTransportes() {
		...
		try {
			...
			TypedQuery<Transporte> query = em.createNamedQuery("negocio.TransporteJPA.Transporte.finAll",
					Transporte.class);
