...
public class VEliminarProducto extends JFrame implements IGUI {
	...
	private void cerrarTodasLasVentanasDeEsteipo() {
		for (Window window : Window.getWindows()) {
			if (window instanceof VEliminarProducto && window.isVisible()) {
				window.dispose();
			}
		}
	}
	...
	public void actualizar(Context context) {
		...
		case RES_QUITAR_PRODUCTO_VENTA_KO:
			cerrarTodasLasVentanasDeEsteipo();
			JOptionPane.showMessageDialog(null, "No se pudo eliminar el producto del carrito. Compruebe los datos.");
			Controlador.getInstancia().accion(new Context(Evento.PASAR_CARRITO_A_CERRAR, carrito));
			break;
		default:
			break;
