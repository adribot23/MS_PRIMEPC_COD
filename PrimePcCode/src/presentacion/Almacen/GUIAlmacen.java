 ...
 * case RES_ALTA_ALMACEN_OK: JOptionPane.showMessageDialog(null,
 * "Almacen dado de alta con ID: " + (int) datos); break; case
 * RES_ALTA_ALMACEN_KO: JOptionPane.showMessageDialog(null,
 * "Error al dar de alta el almacen."); break; case RES_BAJA_ALMACEN_OK:
 * JOptionPane.showMessageDialog(null, "Almacen dado de baja correctamente.");
 * break; case RES_BAJA_ALMACEN_KO: JOptionPane.showMessageDialog(null,
 * "Error al dar de baja el almacen."); break; case RES_MODIFICAR_ALMACEN_OK:
 * JOptionPane.showMessageDialog(null, "Almacen modificado correctamente.");
 * break; case RES_MODIFICAR_ALMACEN_KO: JOptionPane.showMessageDialog(null,
 * "Error al modificar el almacen. Verifica los datos."); break; case
 * RES_BUSCAR_ALMACEN_OK: JOptionPane.showMessageDialog(null, (TAlmacen) datos);
 * break; case RES_BUSCAR_ALMACEN_KO: JOptionPane.showMessageDialog(null,
 * "Almacen no encontrado."); break; case RES_MOSTRAR_TODOS_ALMACENES_OK:
 * mostrarTablaAlmacenes((Collection<TAlmacen>) datos); break; case
 * RES_MOSTRAR_TODOS_ALMACENES_KO: JOptionPane.showMessageDialog(null,
 * "No hay almacenes para mostrar."); break; default:
 * JOptionPane.showMessageDialog(null, "Evento no reconocido: " + evento); } }
 * 
 * private void mostrarTablaAlmacenes(Collection<TAlmacen> almacenes) { String[]
 * columnNames = { "ID", "Nombre", "Capacidad Máxima", "Ocupación", "Activo" };
 * Object[][] tableData = new Object[almacenes.size()][columnNames.length]; int
 * i = 0; for (TAlmacen a : almacenes) { tableData[i][0] = a.getId();
 * tableData[i][1] = a.getNombre(); tableData[i][2] = a.getCapacidadMaxima();
 * tableData[i][3] = a.getOcupacion(); tableData[i][4] = a.getActivo(); i++; }
 * JTable table = new JTable(tableData, columnNames);
 * table.setFillsViewportHeight(true);
 * table.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
 * table.setEnabled(false); JScrollPane scrollPane = new JScrollPane(table);
 * JOptionPane.showMessageDialog(null, scrollPane, "Almacenes",
