...
public class VAnyadirPaquete extends JFrame implements IGUI {
	private static final long serialVersionUID = 1L;

	private final JTextField idPaqueteField = new JTextField();
	private TCarritoFactura carrito;

	public VAnyadirPaquete() {
		super("Añadir paquete a factura");
		initGUI();
	}

	private void initGUI() {
		setLayout(new GridLayout(3, 2, 10, 10));
		getRootPane().setBorder(BorderFactory.createTitledBorder("Añadir paquete a factura"));
		...
		JButton aceptar = new JButton("Añadir paquete");
	...
	private void onAceptar() {
		try {
			...
			if (idPaquete <= 0) {
				JOptionPane.showMessageDialog(this, "El Id paquete debe ser un número positivo.",
			...
			if (carrito != null) {
				...
				Controlador.getInstancia().accion(new Context(Evento.INSERTAR_PAQUETE_FACTURA, carrito));
				limpiarCampos();
			}

		} catch (NumberFormatException ex) {
			JOptionPane.showMessageDialog(this, "El Id paquete debe ser un numero entero positivo.",
					"Datos incorrectos", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void cerrarTodasLasVentanasDeEsteipo() {
		for (Window window : Window.getWindows()) {
			if (window instanceof VAnyadirPaquete && window.isVisible()) {
				window.dispose();
			}
		}
	}
