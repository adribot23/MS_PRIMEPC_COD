...
public class VVerProdPorProveedor extends JFrame implements IGUI {
	...
	private void initGUI() {
		...
		getRootPane().setBorder(BorderFactory.createTitledBorder("Mostrar productos que vende un proveedor"));
		...
		btnMostrar.addActionListener(e -> {
			try {
				int id = Integer.parseInt(idProveedor.getText().trim());
				Controlador.getInstancia().accion(new Context(Evento.MOSTRAR_PRODUCTOS_POR_PROVEEDOR, id));
				idProveedor.setText("");
	...
	private void mostrarTabla(Set<TProducto> productos) {
		...
		if (productos == null || productos.isEmpty()) {
			JOptionPane.showMessageDialog(this, "No hay productos asociados a este proveedor.");
			return;
		}
		...
		JOptionPane.showMessageDialog(null, scrollPane, "Productos del proveedor", JOptionPane.PLAIN_MESSAGE);
	}
}
