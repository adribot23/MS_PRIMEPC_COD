...
public class DAOLineaVentaImp implements DAOLineaVenta {
	...
	public int create(TLineaVenta lineaVenta) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("INSERT INTO LINEAVENTA (ID_PRODUCTO, ID_VENTA, NUM_UNIDADES, PRECIO_UNIDADES) VALUES (?, ?, ?, ?)");
	...
	public TLineaVenta read(TLineaVenta tlineaVenta) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT ID_PRODUCTO, ID_VENTA, NUM_UNIDADES, PRECIO_UNIDADES FROM LINEAVENTA WHERE ID_PRODUCTO = ? AND ID_VENTA = ? FOR UPDATE");
			ps.setInt(1, tlineaVenta.get_producto());
			ps.setInt(2, tlineaVenta.get_venta());
	...
	public Set<TLineaVenta> read_all(int id_venta) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT ID_PRODUCTO, ID_VENTA, NUM_UNIDADES, PRECIO_UNIDADES FROM LINEAVENTA WHERE ID_VENTA = ? FOR UPDATE");
	...
	public int update(TLineaVenta tlineaVenta) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("UPDATE LINEAVENTA SET NUM_UNIDADES = ?, PRECIO_UNIDADES = ? WHERE ID_PRODUCTO = ? AND ID_VENTA = ?");
	...
	public TLineaVenta read_all_by_producto(int id_producto) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT ID_PRODUCTO, ID_VENTA, NUM_UNIDADES, PRECIO_UNIDADES FROM LINEAVENTA WHERE ID_PRODUCTO = ? FOR UPDATE");
